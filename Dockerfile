...
ARG USER_NAME="eric-oss-pm-stats-exporter"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-pm-stats-exporter-app.jar
...
-Dcom.sun.management.jmxremote.access.file=/jmx/jmxremote.access -jar eric-oss-pm-stats-exporter-app.jar
...
LABEL \
    org.opencontainers.image.title=eric-oss-pm-stats-exporter-jsb \
